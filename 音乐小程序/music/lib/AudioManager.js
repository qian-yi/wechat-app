...
export default class AudioManager{
  ...
  static setSong(song,songs){
	//获取点击歌曲的url
	const songurl = request.music + song.songid +"&songmid="+ song.songmid;
	...
	wx.request({
		...
		success(res){
			//播放器的属性
			const audioAttr = {
				src: res.data[0],//歌曲的url
				title: song.songname,//歌曲名称
				epname: song.songorig,//专辑名称
				singer: song.singer[0].name || song.singer,//歌手名称
...
/*============= 通过页面代理触发的函数 this指向页面实例 =============*/ 
  ...
  //显示聊天界面
  static chat(){
	  const singer = this.data.playerSong.singer[0].name || this.data.playerSong.singer;
    //打开子页面的形式跳转
    wx.navigateTo({
		url: '/pages/player/chat?' + utlis.objDeUCode(this.data.playerSong) +"&singer="+ singer
    });
  }
  ...
  static download(){
    //需要下载的路径
    const url = request.music + this.data.playerSong.songid +"&songmid="+ this.data.playerSong.songmid;
    ...
    //监听一个下载进度事件
    files.onProgressUpdate((file)=>{
      	console.log(`已下载 ${file.progress}%` );
    });
  }
